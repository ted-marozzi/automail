...
public class MailGenerator {
    ...
    private final Map<Integer, ArrayList<DeliveryItem>> allMail;
    ...
    private DeliveryItem generateMail(){
        ...
        if(	(random.nextInt(6) > 0) ||  // Skew towards non priority mail
        	...
        	allMail.get(arrivalTime).stream().anyMatch(e -> e instanceof PriorityMailItem)))
        {
            /* Generates food items approx 50% of the time if enabled in automail.properties */
            if(Simulation.getDELIVER_FOOD() && random.nextFloat() < percentageFood)
    ...
    public void generateAllMail(){
        while(!complete){
            ...
            if(allMail.containsKey(timeToDeliver)){
                 ...
                 * that time step.
                 */
                ArrayList<DeliveryItem> newMailList = new ArrayList<>();
